...
void drawmodel(void) {
    if (!pmodel) {
        ...
        // 모델 파일 경로 디버깅 메시지 추가
        printf("Attempting to load model from: ./data/porsche.obj\n");

        // glmReadOBJ 호출 시 명시적으로 경로 지정
        pmodel = glmReadOBJ("/Users/yourusername/path_to_project/data/soccerball.obj");        if (!pmodel) {
            fprintf(stderr, "Failed to load model from ./data/porsche.obj\n");
            exit(0);
        }
...
#include <unistd.h> // getcwd()를 사용하기 위한 헤더
